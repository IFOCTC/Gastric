...
df_final_plot <- merge(df_condition_plot, df_line_gastric_plot,
                       by = "ID")
## GENE
ggplot(df_final_plot, aes(x = ID, y = log2(as.numeric(CLDN18) + 1))) +
  geom_segment(aes(x = ID, xend = ID,
                   y = 0, yend = log2(as.numeric(CLDN18) + 1)),
               color = "grey", size = 1.5) +  
  ...
  labs(title = "CLDN18", subtitle = "IRE Cohort Analysis",
       ...
       y = "Gene Expression",
       fill = "") +
  scale_color_manual(values = c("SENSITIVE" = "#1f77b4",  
                                "RESISTANT" = "#d62728")) +
  ylim(0, max(log2(as.numeric(df_final_plot$CLDN18) + 1))) +
